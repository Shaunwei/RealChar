...
export default function MicrophoneControl({ isDisabled, handleMic }) {
  const { microphoneList, selectedMicrophone, handleMicrophoneSelect } =
    useAppStore();

  return (
    <>
      <div className='flex md:hidden'>
        <Button isIconOnly variant='light' onPress={handleMic}>
          {!isDisabled ? (
            <Image priority src={microphoneSVG} alt='microphone' />
          ) : (
            <Image priority src={micOffSVG} alt='disabled mic' />
          )}
        </Button>
      </div>
      <div className='hidden md:flex'>
        <ButtonGroup radius='full' variant='light'>
          <Tooltip content='Microphone'>
            <Button isIconOnly className='hover:bg-button' onPress={handleMic}>
              {!isDisabled ? (
                <Image priority src={microphoneSVG} alt='microphone' />
              ) : (
                <Image priority src={micOffSVG} alt='disabled mic' />
          ...
          >
            <DropdownTrigger aria-label='Dropdown trigger'>
              <Button isIconOnly className='min-w-[20px] w-5'>
                <Image
                  priority
                  src={arrowSVG}
                  alt='arrow'
