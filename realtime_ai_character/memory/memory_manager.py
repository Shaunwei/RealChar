...
class MemoryManager(Singleton):
    ...
    async def process_session(self, user_id: str, session_id: str,
        ...
        if not generated_memory or generated_memory.lower() == 'no info':
        ...
        if 'postgres' in os.environ.get('DATABASE_URL'):
            ...
            if query_similar:
                logger.info(
                    "skip generating memory due to too similar to existing ones")
                return
        await asyncio.to_thread(memory.save, self.sql_db)
        logger.info(
            f"Memory generated for user {user_id} and session {session_id}.")
